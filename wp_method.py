...
class WPDB():
    def __init__(self, site=SITE, user=SITE_USER, passwd=SITE_PASSWD):
        try:
            self.db=pymysql.connect(host=WP_HOST,port=WP_PORT,password=WP_PASSWD,user=WP_USER,database=WP_DB,charset='utf8')
    ...
    def create_post(self,title,content,post_format='image',tag=None,category=None,thumnnail_path=None):
        post = WordPressPost()
        post.title = title
    ...
    def AlterTime(self,post_id,new_time):
        sql='update wp_posts set `post_date`="{}" where `ID`={};'.format(new_time,post_id)
        self.cur.execute(sql)
        self.db.commit()
...
if __name__=='__main__':
    ...
    wp.AlterTime(16,'2018-10-01 16:39:44')
